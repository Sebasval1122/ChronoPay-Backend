...
def enviar_documento(documento_firmado):
    ...
    contenido = documento_firmado.encode("utf-8") if isinstance(documento_firmado, str) else bytes(documento_firmado)
    request = Request(url, data=contenido, headers={"Content-Type": "application/xml"}, method="POST")
    try:
